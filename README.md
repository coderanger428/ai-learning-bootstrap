...
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | -------- |
| P01 | **Linear Regression (NumPy)** – Predict continuous targets (e.g., house prices) using gradient descent and visualize the best-fit line/plane; implement cost function, parameter updates, and convergence behavior manually                                |      | complete |
| P02 | **Logistic Regression (NumPy)** – Predict binary outcomes (e.g., customer churn yes/no) using sigmoid activation and gradient descent; implement classification boundary, probability outputs, and training visualization from scratch                     |      | complete |
| P03 | **MLP From Scratch (NumPy)** – Predict student exam scores using multiple mixed features (categorical + numerical); implement fully connected layers, forward/backward propagation, SGD training loop, and training/validation loss visualization manually |      | complete |
| P04 | **MNIST Classifier (NumPy)** – Classify handwritten digits (0–9) using a fully connected MLP implemented purely in NumPy; include data normalization, batching, training loop, evaluation, and accuracy/loss visualization                                 |      |          |
| P05 | **MNIST Classifier (PyTorch MLP)** – Reimplement MNIST digit classification using PyTorch; focus on tensor operations, automatic differentiation, model definition, training loop, and evaluation pipeline                                                 |      |          |
| P06 | **MNIST CNN (PyTorch)** – Classify handwritten digits using a Convolutional Neural Network; implement convolution layers, pooling, feature extraction, training, evaluation, and performance visualization                                                 |      |          |
| P07 | **Tabular ML Benchmark Project** – Train and compare Linear Regression, Random Forest, and MLP models on the same structured dataset; analyze performance, generalization, and model suitability for tabular data                                          |      |          |
| P08 | **Simple Image Classifier** – Classify images (e.g., dogs vs cats) using a CNN in PyTorch; implement dataset loading, augmentation, training, evaluation, and prediction visualization                                                                     |      |          |
| P09 | **Tiny Object Detector** – Train a lightweight object detection model to detect simple objects (e.g., cups, books, bottles); implement bounding box prediction, classification heads, training loop, and visualization of detections                       |      |          |
| P10 | **Object Counter System** – Count objects in images/video using detection + tracking; combine a detector with object tracking to produce real-time counting and annotated video output                                                                     |      |          |
